...
When upgrading from Harvester v1.7 to v1.8, you will encounter an operating system image corruption issue if _all_ of the following conditions are met:
...
The root cause is [a bug in Harvester v1.4.1 and earlier](https://github.com/harvester/harvester/issues/7493). If the system was originally configured to use a separate data disk, this bug caused the `COS_STATE` partition size to be set to 8G at installation time, rather than the expected 15G. This lack of space causes trouble during subsequent upgrades. In particular, after upgrading from v1.7.x to v1.8.x, the `active.img` file, which stores the operating system image, will be subtly corrupted, for example [files may filled with all zeros rather than their expected content](https://github.com/harvester/harvester/issues/10687). This in turn results in unexpected system failures.
...
This can be done manually by logging in via `ssh`, then using `lsblk` to check the partition sizes. Here's what you'd typically see on an affected node:
...
Or, more briefly, on a node that is affected:
```
# lsblk -o NAME,SIZE,LABEL | grep COS_STATE
├─sda4     8G COS_STATE
```

If the node is _not_ affected your will see 15G:
```
# lsblk -o NAME,SIZE,LABEL | grep COS_STATE
├─sda4     15G COS_STATE
```

Alternately, if you run the latest version of the [pre-check script](https://github.com/harvester/upgrade-helpers/tree/main/pre-check) on a v1.7.x control plane node, this will automatically check all nodes. When run on an affected system the output will be similar to the following:
...
To future-proof an affected system, the best course of action is to make the `COS_STATE` partition be 15G. This ensures there will be plenty of free space going forwards for any future upgrades. There are two ways of doing this:

1. Reinstall the current version of Harvester on all affected nodes.
2. Use a tool like GParted to resize the `COS_STATE` and `COS_PERSISTENT` partitions and their filesystems in-place.
...
If it is not possible to do either of the above, a stop-gap measure is to delete the `passive.img` file from the `COS_STATE` partition prior to upgrade, in order to free up enough space for the upgrade to succeed without corrupting the `active.img` file. This will work for Harvester v1.7 to v1.8 upgrades, but is not a permanent solution to the problem.

All the above options are detailed below.

#### Reinstall Harvester on all affected nodes

This option requires a Harvester cluster with three active control plane nodes, plus ideally at least one more node that can be promoted to a management role temporarily.

For each affected node:

1. Delete that node [using the procedure in the Harvester documentation](https://docs.harvesterhci.io/v1.8/host/#deleting-a-node).
   - If this is a control plane node, and there are any non-control plane nodes with either the Default or Management role, Harvester will automatically promote one of these other nodes, to ensure that there are three control plane nodes at all times.
2. Reinstall the same version of Harvester on the node you just deleted, either by booting the ISO image and working through the installer interactively, or by PXE. You'll need to tell the installer to join the existing cluster. Otherwise, use the same configuration as the node currently has (hostname, network, role, data disk, etc.)
   - Reinstalled nodes may require further post-installation configuration and customisation.
   - Because this is a reinstall, the Longhorn default data disk will be wiped. Any additional disks that were added separately via the Harvester UI after initial install will not be touched (**TBD additional disks may require extra handling, see testing note below**).
3. Once the node above has successfully joined the cluster, go back to step 1 and repeat for the next node.

Bear in mind that if all nodes in the cluster have the Default or Management role, that after doing the above, you may end up with a different set of nodes running the control plane than when you started, depending on which nodes are promoted, and when. If you need finer control over this, and if roles weren't already specified during initial installation (or they need chaning), you can add labels to nodes to define their roles before doing the above. Nodes labelled `node-role.harvesterhci.io/management=true` will be consiered for promotion, while nodes labelled `node-role.harvesterhci.io/worker=true` will not.
...
The upgrade process when going from Harvester v1.7 to v1.8 uses about 4.3G of disk space on the `COS_STATE` partition while it's generating the new `active.img` file. By deleing the `passive.img` file, in the above example we end up with 5.5G free, so the upgrade will succeed.
...
Bear in mind also that as Harvester's OS image has tended to increase slightly in size from release to release, that there may come a time in future when deleting `passive.img` still won't result in enough free space. This is why the future-proof options are to either reinstall or repartition affected nodes in-place.
...
1. Reboot into fallback mode (the second grub entry), which will be running SL Micro 6.1 from Harvester v1.7.x. NetworkManager won't start at this point, so you have to log in on the console and run `rm /etc/systemd/system/dbus.service` to remove a symlink which is put there by SL Micro 6.2 from Harvester v1.8.x. Reboot into fallback mode again, and you'll have networking running correctly again.
...
2. Log in via `ssh` or on the console and become root, then run the following commands:
   ...
   # mount -o remount,ro /run/initramfs/cos-state
   ```
   This copies the previous (clean) `passive.img` (SL Micro 6.1/Harvester v1.7.x) over the corrupted `active.img` and the label is set correctly so it will boot.

   We also move the `passive.img` to the `/root` directory to free up space on the `COS_STATE` partition. We do that rather than just deleting `passive.img` because if anything goes wrong with the following procedure we may need to put it back and retry.

3. Reboot and select the first entry on the grub screen. This will say it's booting Harvester v1.8.x, but will actually boot into the reverted SL Micro 6.1 image from Harvester v1.7.x.

4. Copy `rootfs.squashfs` from the Harvester v1.8.x ISO to a convenient location, or download it from https://releases.rancher.com/harvester/v1.8.0/harvester-v1.8.0-rootfs-amd64.squashfs or https://releases.rancher.com/harvester/v1.8.1/harvester-v1.8.1-rootfs-amd64.squashfs.
...
5. As root, run the following commands:
   ...
   # elemental upgrade \
       ...
       --debug
   ```
   - This will create a new `active.img` generated based on the v1.8.x rootfs.
...
7. Don't worry about the `/etc/systemd/system/dbus.service` symlink that was removed in step 1. It it will come back automatically when then new v1.8.x active.img boots.
...
When you boot a Harvester node, by default it will run from `active.img`.  The `passive.img` file is only ever ordiniarly used as a fallback if `active.img` can't boot for some reason (`passive.img` is what boots if you select the Fallback option on the grub boot screen).

During upgrade, Harvester calls out to `elemental upgrade`, which will generate a new `active.img` based on the rootfs from the new Harvester version, as follows:
...
The `transition.img` created is a sparse file on the `COS_STATE` partition. This image is then loopback mounted, and `elemental` runs `rsync` to copy all the necessary OS files to that image. Because this is a loopback mounted image, from the perspective of `rsync` and `elemental` it just looks like a block device with plenty of free space, so the `rsync` will always succeed. It's the Linux kernel which is responsible for writing the data out to the underlying image file on the `COS_STATE` partition, which happens separately. If that partition runs out of space, you'll see I/O errors in the system journal, but the upgrade process has no way of knowing that anything has gone wrong. Here's an example of what the I/O errors look like. If you see this sort of thing during upgrade, it means you've encoutered the problem.
